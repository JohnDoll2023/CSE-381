...
void producer(std::vector<int>& q, std::mutex& qMutex,
    ...
    for (int i = 0; i < n; i++) {
        ...
        // add i to queue
        q.push_back(i);
        // notify other threads that we finished adding to the queue
        qCondVar.notify_all();
    }
}
...
std::vector<int> countDigits(const std::vector<int> values, int n) {
    ...
    // create vector of mutex's 
    std::vector<std::mutex> lock(10);
    ...
    auto thrMain = [&](int id) {
        for (size_t i = id; (i < values.size()); i += n) {
            ...
            // lock thread at roomNum
            lock.at(roomNum).lock();
            increment(room[roomNum]);
            // unlock thread at roomNhum
            lock.at(roomNum).unlock();
        }
    };
